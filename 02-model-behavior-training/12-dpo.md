...
- DPO exists because [RLHF](11-rlhf.md) is a heavy, unstable three-stage pipeline (labeling → **reward model** → **RL loop**), and two of those stages are means, not ends. DPO reaches the same goal from the **same preference pairs** while deleting the reward model and the RL loop.
