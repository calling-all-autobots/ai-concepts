...
[Fine-tuning](07-fine-tuning.md) works — you take a capable base model and keep training it on your own examples until it behaves the way your product needs. But done the obvious way, it is brutally expensive, and the expense shows up in three separate places.
...
And a boundary that trips people up: PEFT adapts *behavior*, it doesn't reliably stuff in large amounts of *new factual knowledge*. If the base model simply doesn't know your company's current data, the fix is usually retrieval, not any flavor of fine-tuning — that argument lives in [Fine-tuning](07-fine-tuning.md) and [RAG](../04-retrieval-knowledge/21-rag.md), and PEFT doesn't change it.
...
- **PEFT is a family:** adapters (modules between layers), prompt/prefix tuning (learned vectors prepended, weights untouched), and **[LoRA](09-lora.md)** — the flagship you name by default. Don't conflate PEFT (the family) with LoRA (one member).
