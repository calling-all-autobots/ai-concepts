...
Start from the two-stage picture in [Pretraining vs. post-training](../01-foundations/06-pretraining-vs-posttraining.md): pretraining bakes in knowledge, post-training shapes behavior. **Fine-tuning is post-training you run yourself, after the provider is done.** The provider already turned a base model into a helpful assistant; you take that assistant and give it one more, much smaller round of training on examples of exactly the behavior you want — 500 support tickets answered in your brand voice, 2,000 documents converted to your exact JSON schema, a few thousand examples of your legal team's summarization style.
...
This is the point to connect back to architecture. Recall from [Transformers](../01-foundations/04-transformers.md) that a model's learned facts live mostly in its dense feed-forward weights, while much of what fine-tuning most easily moves is the *style and format* of the output. A small fine-tuning dataset is a strong, repeated signal about *how* to respond and a weak, sparse signal about *new individual facts* — which is the mechanical reason behind the failure mode we'll keep returning to.
...
Why? Knowledge, from the [pretraining lesson](../01-foundations/06-pretraining-vs-posttraining.md), is overwhelmingly a pretraining phenomenon, and facts live in the dense feed-forward weights of the network. A small fine-tuning dataset is a strong signal about *patterns of expression* but a weak, sparse channel for *individual facts*. So if you fine-tune a model on your company's internal documents hoping it will "learn" them, what actually happens is it gets fluent at *sounding* like your docs — the vocabulary, the structure, the tone — while still confidently making up specifics in the gaps. It absorbs the style and hallucinates the facts.

The right tool for injecting fresh, private, or frequently-changing facts at answer-time is retrieval — fetch the relevant passages and put them in the prompt so the model reads them each time, rather than trying to write them into the weights. That's **RAG (Retrieval-Augmented Generation)**, covered in [RAG](../04-retrieval-knowledge/21-rag.md).
...
- **Serving** — a full fine-tune produces a whole separate model to host; PEFT/LoRA adapters are small and can often be served more cheaply alongside a shared base model. For the broader pricing picture, see [Cost and unit economics](../10-production-ops/51-cost-unit-economics.md).
...
- The tightest framing of the failure mode: **fine-tuning reliably shapes *behavior*; it does not reliably inject *knowledge*.** Behavior gap → fine-tune; knowledge gap → retrieve ([RAG](../04-retrieval-knowledge/21-rag.md)). Fine-tune a model on your docs and it learns to *sound* like them while still inventing the specifics.
...
- **Full fine-tuning updates all weights (expensive); the parameter-efficient variants ([PEFT](08-peft.md), [LoRA](09-lora.md)) touch a tiny fraction (cheap)** — and in practice "we fine-tuned it" almost always means the cheap kind.
