...
**Reproducibility** is the ability to get the same behaviour tomorrow that you got today. It's what makes an eval score mean something — if the model can change under you, a 96% from last week doesn't tell you anything about this week. It's what makes a tuned prompt safe — prompts are famously brittle across model versions, and phrasing that a model was practically trained to love can land differently on the next one (that fragility has its own lesson: [prompt engineering](../05-prompting/28-prompt-engineering.md)). And it's what makes incidents debuggable — if behaviour drifts, you want to know it was *your* change, not a silent swap. A dated snapshot buys all of this.
...
- **Pinning is not determinism.** It removes the *version* variable only; the same snapshot still varies output because of [sampling](../03-reasoning-generation/16-sampling-decoding.md). Stable behaviour needs a pinned snapshot *and* deterministic decoding — and even then, "consistent," not "identical."
