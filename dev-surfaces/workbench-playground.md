...
Strip it down and a workbench is three panes: a place to write the prompt (usually a system prompt plus a user message), a Run button, and a place the response appears — wrapped around a plain API call the page makes for you. Around that core sit conveniences: a dropdown to pick the model, sliders for the [generation parameters](../reasoning-generation/sampling-decoding.md), a token counter, a way to save or share a prompt, and often a "view code" button that emits the equivalent API snippet so an engineer can lift your working prompt straight into the app.
...
**The surrounding context differs — and this is the big one.** In the workbench you type a clean, isolated prompt. In production that same prompt is a *template* that gets filled at runtime with retrieved documents, conversation history, tool outputs, and user text you never previewed — all of it competing for room in the [context window](../foundations/context-windows.md). The polished instruction you tested in a vacuum now sits buried under three pages of injected content, and real user inputs are messier, longer, and more adversarial than the tidy example you typed. You tested the recipe; production serves it with substitute ingredients you never tasted.
...
For the actual *craft* of writing the prompt you iterate on here — structure, examples, instruction design — see [prompt engineering](../prompting/prompt-engineering.md); this lesson is about the *surface* you do that work on, not the techniques.

## Summary / Points to Remember

- A **workbench** (Anthropic) or **playground** (OpenAI) is a no-code web UI — a prompt box, a Run button, and controls for model, [temperature and sampling params](../reasoning-generation/sampling-decoding.md) — for iterating on prompts interactively. Its entire value is a *fast feedback loop*.
...
- The gap has four sources: the **parameters** differ (temperature, max tokens, hidden system prompt), the **model version** differs (dropdown default vs. pinned snapshot), the **surrounding context** differs (clean prompt vs. a template stuffed with retrieved docs, history, and messy user input filling the [context window](../foundations/context-windows.md)), and there's **no eval or regression safety net** — you eyeballed one sample of a model that answers differently every call.
