...
[Instruction tuning](instruction-tuning.md) got the model to *follow* instructions — to treat "write a condolence note to a customer" as a command to obey rather than text to autocomplete. But following the instruction is not the same as giving the answer a person would actually *prefer*. For almost any real prompt there are thousands of valid completions, and they differ in ways that decide whether the product is good: which is more genuinely helpful, which strikes the right tone, which is honest instead of confidently wrong, which declines a bad request gracefully instead of lecturing.
...
- It comes *after* [instruction tuning](instruction-tuning.md): instruction tuning makes the model obey; RLHF shapes *how* it obeys, toward what humans actually prefer. Both are [post-training](../foundations/pretraining-vs-posttraining.md).
...
- RLHF is **expensive and complex** (labeling + reward model + unstable RL). That's why the simpler [DPO](dpo.md), which skips the reward model, exists as an alternative.
...
**Interviewer:** [DPO](dpo.md) learns from the same preference data without a separate reward model or an RL loop. So why hasn't it just replaced RLHF?
