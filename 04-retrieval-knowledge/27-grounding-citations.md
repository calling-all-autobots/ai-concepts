...
You've done everything right up the pipeline. You [chunked](22-chunking.md) the documents, searched the [vector store](23-vector-databases.md), ran a good [retrieval](24-retrieval.md) pass, and the three passages sitting in the [context window](../01-foundations/05-context-windows.md) genuinely contain the answer. Then the model answers — and it answers *wrong*, or it answers right but from some fact it clearly pulled out of its own memory rather than the passage you handed it, or it gives a beautiful answer with a citation to "Document 2" that, when you actually open Document 2, says nothing of the kind.

Here is the uncomfortable truth this lesson exists to fix: **putting the right context in the prompt does not mean the answer is derived from it.** Retrieval's job ends when the passages are in the window. Whether the model *actually reads and uses* those passages — instead of ignoring them and answering from its trained-in [parametric memory](../02-model-behavior-training/07-fine-tuning.md), or wandering off into a fluent guess — is a separate problem, and it is the *last mile* of the whole [RAG (Retrieval-Augmented Generation)](21-rag.md) pipeline. That last mile is **grounding** (making the answer provably derived from the retrieved text) and **citations** (attaching, to each claim, a pointer back to the exact source it came from so a human can check it). Get this stage wrong and every upstream investment is wasted: you fetched the right folder and the model answered from a rumor anyway.

> [!NOTE]
> **Parametric memory** — the knowledge baked into a model's weights during [pretraining](../01-foundations/06-pretraining-vs-posttraining.md), as opposed to knowledge supplied at answer-time in the prompt. "Parametric" just means "stored in the parameters (the weights)." The whole point of RAG was to answer from *supplied* text instead of this frozen, unverifiable memory — but the model still *has* that memory and will happily fall back on it unless you actively steer it not to. That tension is the heart of this lesson.

## The one analogy to remember

**The picture:** A newspaper's standards desk. A reporter is only allowed to print what a named, on-the-record source actually told them — no printing from personal hunch, memory, or a rumor overheard in the hallway. Every sentence in the filed story must carry a footnote naming which source it came from. Then a fact-checker goes down the story line by line, and for each footnote, confirms the named source really said that thing.

**The mapping:** the reporter = the model doing [generation](21-rag.md); the rule "print only what your sources told you, never from memory" = **grounding** (answer only from the retrieved passages, not from parametric memory); the footnote on every sentence = **citations** (per-claim attribution back to a source chunk); the fact-checker verifying each footnote actually supports its line = the **citation-accuracy / attribution check**; a source who was on-record but simply *wrong* = a retrieved chunk that is itself wrong (faithful reporting, false story); a reporter who footnotes a source that never said that = **citation hallucination**.

**Why it holds:** journalism draws the exact distinction this lesson turns on — being *faithful to a source* (attributable, checkable) is not the same as being *true* (the source can be wrong). A paper's credibility doesn't come from the reporter being omniscient; it comes from every claim being *traceable to a source the reader can go check*. That traceability is precisely what citations buy a RAG product, and "print only what your sources said" is precisely what grounding enforces.

**Say it like this:** "Grounding is the newsroom rule that you only print what your sources told you; citations are the footnotes that let the reader check each line — together they make the answer *verifiable*, which is not the same as making it *true*."

*Where it breaks:* a seasoned reporter cross-checks several sources and *notices* when one contradicts the others or smells wrong; the model won't, unless you build that in. Hand it a single authoritative-looking passage that happens to be wrong and it will faithfully, confidently report the error with a tidy footnote attached — the analogy's reporter has judgment the model lacks.
...
- **Grounded-but-wrong.** The answer is perfectly faithful to a retrieved chunk that was itself wrong, stale, or irrelevant. This is not a generation failure at all — it's [retrieval](24-retrieval.md) failure propagated downstream. You fix it *upstream* (better chunking, [hybrid search](25-hybrid-search.md), [reranking](26-reranking.md)), not by tuning the grounding prompt. Naming which stage owns the failure is the whole skill.

## Measuring it: groundedness and attribution

You cannot tune what you don't score, and grounding is measurable — you just have to know the two distinct questions to ask. (Keep this light; the machinery of how you build and run these lives in [evaluation methods](../09-evaluation/45-evaluation-methods.md).)
