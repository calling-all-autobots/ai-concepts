...
The [vector database](23-vector-databases.md) lesson ended on a deliberate boundary: the database returns "the nearest vectors by the distance metric," and whether those are *actually the right ones* is somebody else's job. This is that somebody. **Retrieval is the query-time logic that turns a user's question into a set of chunks worth putting in the [context window](../01-foundations/05-context-windows.md)** — embedding the question, searching the store, scoring candidates by how close they are, and deciding how many to keep and whether any clear the bar at all. The vector database is the *warehouse and its index*; retrieval is the *act of shopping* — what you search for, how you rank, how much you take home.

And it is, per [RAG](21-rag.md)'s iron law, the stage that caps everything: the model can only answer as well as the passages retrieval hands it, and most RAG failures are retrieval failures. So this lesson is where that law gets operational — the concrete decisions and the one deep subtlety that separates a retrieval system that looks fine in a demo from one that holds up on real questions.
...
> [!NOTE]
> **What's assumed here.** This lesson stands on three earlier ones and re-teaches none: [embeddings](../01-foundations/02-embeddings.md) (text → a meaning-vector), [chunking](22-chunking.md) (documents pre-split into the units being searched), and [vector databases](23-vector-databases.md) (the store that finds nearest vectors fast). If any is shaky, read it first. Here they're the given, and the question is: *given* a store full of chunk-vectors, how do you query it well?

## The one analogy to remember

**The picture:** A recruiter fills a role. They can't interview everyone, so they rank every résumé by how well it matches the job posting and interview only the top handful. Three things decide whether this works. First, they have to *describe the job in the words résumés actually use* — post it in the wrong vocabulary and great candidates who phrased their experience differently never surface. Second, they must not be fooled by a résumé *stuffed with the right buzzwords* that ranks high but belongs to someone who can't do the job — sounding like a match isn't being one. Third, they pick *how many* to interview: too few and the best candidate is in the pile they skipped; too many and they drown in mediocre ones.

**The mapping:** the job posting = the user's query; each résumé = a stored chunk; ranking résumés by match = scoring chunks by similarity to the query; interviewing the top handful = returning the **top-k** results; describing the job in the résumés' vocabulary = the query-and-document sharing an embedding space (and the vocabulary-mismatch problem when they don't); the buzzword-stuffed résumé that ranks high but can't do the job = a chunk that's *similar* but not *relevant*; choosing how many to interview = the top-k tradeoff.

**Why it holds:** the recruiter's entire process is "rank by a cheap proxy for fit, then take the top few" — which is exactly what retrieval does — and its two signature ways of going wrong are the two that matter here: missing a good candidate who used different words (recall failure from vocabulary mismatch), and being seduced by surface-matching text (the similarity-isn't-relevance trap). The proxy is useful *and* leaky in precisely the same way.

**Say it like this:** "Retrieval is a recruiter ranking résumés — score everything by how well it matches, interview the top few, and never confuse a résumé full of the right words with a candidate who can actually do the job."

*Where it breaks:* a recruiter can read a résumé and *understand* it; retrieval's scoring is a geometric distance between vectors with no comprehension, so it's even more prone to the buzzword trap than a human — which is exactly why later stages ([reranking](26-reranking.md), [hybrid search](25-hybrid-search.md)) exist to catch what a raw similarity score misses.
...
- **[Reranking](26-reranking.md)** attacks *similar-but-not-relevant*: a second, slower, sharper model re-scores the top candidates for true relevance and reorders them, so the genuinely-best rise above the merely-similar.
- **[Hybrid search](25-hybrid-search.md)** attacks *relevant-but-not-similar*: combine semantic search with old-fashioned keyword search so exact terms and identifiers the embedding blurred still get caught.
...
- **Retrieval is the query-time logic that turns a question into ranked chunks:** embed the query, score candidates by similarity, keep the top-k (optionally above a threshold). The [vector DB](23-vector-databases.md) is the store; retrieval is the act of querying it well.
...
- **Those two failures motivate the next lessons:** [reranking](26-reranking.md) fixes similar-but-not-relevant (a sharper model re-scores the top candidates); [hybrid search](25-hybrid-search.md) fixes relevant-but-not-similar (add keyword search for literal matches).
