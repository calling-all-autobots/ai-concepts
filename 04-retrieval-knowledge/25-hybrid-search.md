...
[Retrieval](24-retrieval.md) left us with a named, specific wound. Ranking chunks by geometric closeness in [embedding](../01-foundations/02-embeddings.md) space is a cheap, mostly-good proxy for relevance, and it fails in two directions. Reranking handles one of them. This lesson is the fix for the other: **relevant-but-not-similar** — the passage that genuinely answers the question but sits far from it in embedding space because it's phrased differently, or because the query is an *exact string* that a meaning-vector cannot represent.
...
**The picture:** A vast secondhand bookstore with two clerks. Ada knows every book by what it's *about* — describe a half-remembered plot and she walks you to the right shelf even if you've mangled the title. Cyrus hasn't read a single one, but he has memorized the catalog codes — read him an ISBN (the unique catalog number printed in every book) and he puts his finger on it in seconds. Ada is hopeless with a string of digits; Cyrus is hopeless with "it's about a lighthouse and grief." So you ask *both*, each hands you a ranked shortlist, and you merge the two lists into one.

**The mapping:** Ada = the dense/semantic ([vector](23-vector-databases.md)) retriever; Cyrus = the lexical/keyword (BM25) retriever; "book about a lighthouse and grief" = a paraphrase / vocabulary-mismatch query; the ISBN = an exact identifier (SKU, error code, part number); each clerk's shortlist = one ranked result list; merging the two shortlists = **fusion**; the final merged shortlist = the candidate set you hand downstream.

**Why it holds:** the two clerks fail on *opposite* inputs — one on exact strings, one on fuzzy meaning — so the union of their shortlists has a blind spot only where *both* are blind, which is rare. That complementarity is the entire design reason hybrid search exists; it isn't two searches for redundancy, it's two searches because they're wrong about different things.

**Say it like this:** "One clerk finds books by what they're about, the other by their exact catalog number — ask both and combine their lists, because no single clerk is good at both kinds of question."

*Where it breaks:* a human merging two shortlists uses judgment; real fusion is a blunt mechanical formula, and — the seam that actually matters — you can't just staple the two lists together, because Ada's "how sure am I" and Cyrus's "how sure am I" are measured on totally different scales. Reconciling those two scales is the one genuine subtlety, and it's why the fusion method below is what it is.
...
**Hybrid genuinely helps when your queries carry exact tokens that must match literally:** product catalogs (SKUs), technical support (error codes, log strings), code search (function names, flags), legal and medical (case numbers, drug names, statute cites), and anywhere users paste proper nouns or identifiers. If your failure logs show "the answer was in the docs but only findable by an exact term the vector search blurred," that's the signature — the [relevant-but-not-similar](24-retrieval.md) failure with an exact-match flavor.
...
So the honest tradeoff sheet: hybrid buys you coverage of the exact-match blind spot, at the cost of a **second index to build and keep in sync**, **added system complexity**, and — if you go the weighted route — **a fusion parameter to tune**. Worth it when your query mix has literal tokens; over-engineering when it doesn't. As always in this category, **measure it** ([Recall@k](24-retrieval.md) with and without the sparse side) rather than adding it on faith.

## Summary / Points to Remember

- **Hybrid search = run a dense/semantic retriever and a sparse/keyword (BM25) retriever, then fuse their two ranked lists into one candidate set.** It's the fix for [retrieval](24-retrieval.md)'s *relevant-but-not-similar* failure.
...
- **Hybrid improves the *candidate set* (recall); it does not decide final order — that's [reranking](26-reranking.md).** Hybrid catches the right chunk; reranking floats the best ones to the top.
