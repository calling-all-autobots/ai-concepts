...
> [!NOTE]
> **Where this sits in the pipeline:** chunking is the *ingestion*-time step — done once when you add a document to the knowledge base, before any question is asked. Each chunk is then turned into an [embedding](../01-foundations/02-embeddings.md) (a numeric fingerprint of its meaning) and stored in a [vector database](23-vector-databases.md) so it can be found later. This lesson is only about *how you cut the document*; how the pieces are stored and searched are the next two lessons. Embedding is covered in [Embeddings](../01-foundations/02-embeddings.md); hold it here as "each chunk gets a meaning-fingerprint used to match it to questions."

## The one analogy to remember

**The picture:** You're preparing a thick textbook for a study group by cutting it into flashcards. Cut too coarse — a whole chapter per card — and every card is technically "about" ten things, so when someone asks about one of them the card is mostly noise and hard to match. Cut too fine — one clause per card — and a card reads "...which then reverses the charge" with no idea of *what* reverses *what* charge; it's a fragment nobody can use. The useful cards are the ones cut at natural seams — one coherent idea per card, enough context to stand alone, not so much that it's about everything.

**The mapping:** the textbook = a source document; cutting into flashcards = chunking; each flashcard = a chunk; a card that's a whole chapter = an oversized chunk (diluted, matches weakly); a card that's one dangling clause = an undersized chunk (no standalone meaning); cutting at natural seams = splitting on structure (paragraphs, sections) instead of blindly by length; a fact spread across two cards = an answer split across chunk boundaries.

**Why it holds:** a flashcard's usefulness is decided entirely by *where you cut*, not by how well you search the deck afterward — the same cut that makes a card findable and self-contained is the cut that makes a chunk retrievable and self-contained, and the failure of a mid-idea cut (a fragment that means nothing alone) is exactly the failure of a chunk boundary that slices an answer in half.

**Say it like this:** "Chunking is cutting the book into flashcards — one coherent idea per card, cut at the seams, or you get cards that are either about everything or about nothing."

*Where it breaks:* a human making flashcards understands the content and naturally cuts at meaning; a naive chunker cutting every N characters does not — it'll happily slice mid-sentence. So the analogy is the *goal*; most of this lesson is about how to approximate "cut at the seams" mechanically.
...
- **Citations.** [Grounding and citations](27-grounding-citations.md) needs to tell the user *where* an answer came from; the source lives in the chunk's metadata.
