...
The raw reasoning chain is usually **kept hidden** from the user (shown as a summary, if at all), for the UX and safety reasons covered under [chain-of-thought](17-chain-of-thought.md) — the point here is that a reasoning model *generates a lot of it* by design.
...
- **Parallel sampling + selection.** Generate *many* independent attempts and pick or vote among them — [self-consistency](17-chain-of-thought.md) (majority vote over sampled chains) is the classic example. This spends compute *in parallel* rather than in one long line.
...
- **Test-time compute** = spend more computation at inference to get a better answer. It's the [chain-of-thought](17-chain-of-thought.md) insight promoted to a *scaling principle* with an **effort/budget dial**.
- **Reasoning models** are same-architecture transformers **post-trained (typically with RL on checkable tasks like math and code) to reason effectively** and to use a thinking budget well — capability you *buy at inference*, not only bake in at training. Contrast with [RLHF](../02-model-behavior-training/11-rlhf.md): reward is often objective **correctness**, not human preference.
