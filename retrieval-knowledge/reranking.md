...
**That second pass is reranking.** It is the targeted cure for similar-but-not-relevant, and it's the reason the [Retrieval](retrieval.md) lesson told you to retrieve a *generous* `k` for recall and then "cut it back down" — this lesson is the cutting.

> [!NOTE]
> **What's assumed here.** This lesson stands on [retrieval](retrieval.md) (embed the query, score candidates by similarity, keep the top-k) and [embeddings](../foundations/embeddings.md) (text → a meaning-vector). It also leans on one intuition from [attention](../foundations/attention.md): a transformer can let every token *attend to* — weigh its relevance against — every other token it's fed at once. That mechanism is the entire difference between the two model types below, so if it's fuzzy, skim that lesson first. Everything else is introduced as it appears.

## Profile cards get you a shortlist; dates decide

A matchmaker runs a city-wide dating service. Every client fills out a profile card *once*, and the cards sit filed in a cabinet. When you sign up, the matchmaker compares your card against all the others — thousands, in seconds, because the cards were written in advance — and hands you a shortlist of twenty whose cards line up well with yours. But a card is not a partner. So you go on actual *dates* with the shortlisted few, and only in the room, two real people reacting to each other, does it become clear who you actually click with. You reorder your ranking by the dates, not the cards.

Those are the two stages of retrieval-then-reranking, and they differ in the exact way that matters. A profile card is written *alone, ahead of time*, so cards can be filed and compared by the thousand for pennies — but a card can't capture chemistry. That's the first-stage retriever (a bi-encoder): it encodes each document separately and in advance, so searching millions of chunks is a cheap precomputed lookup, but it only ever compares two finished summaries — great on paper, no way to feel the spark. A date puts *both people in the same room reacting to each other* — far more revealing, but it has to happen live, one pairing at a time. That's the reranker (a cross-encoder): it reads your query and one candidate *together*, so it can judge whether this passage actually answers this question — but nothing can be precomputed, so you can only afford it on the shortlist. Hence the whole shape: screen the entire city cheaply by card, then *date* only the twenty.

One line to carry out of here: **retrieval is matching dating profiles to get a shortlist; reranking is going on the actual dates — slower and pricier, but the only way to tell who looks good on paper from who you truly click with.** Where the picture leaks — and this is reranking's single most important limit: a perfect partner the matchmaker never put on your shortlist never gets a date, and no amount of dating fixes that. Reranking only reshuffles the candidates retrieval already surfaced; it can sharpen recall into precision, but it can never recover an answer that was never retrieved.
...
- **Stage one retrieves for recall.** [Retrieval](retrieval.md) casts a wide net — a generous `k`, say 50 or 100 candidates — with one job: make sure the answer-bearing chunk is *somewhere* in the set. It doesn't have to be ranked first; it just has to be present. Recall is "did we catch it at all."
- **Stage two reranks for precision.** The reranker takes that wide set and re-scores every candidate against the query for *true relevance*, then reorders so the genuinely-best land on top. You keep only the few highest — the 3 to 5 that actually go into the model's [context window](../foundations/context-windows.md). Precision is "of what we hand the model, how much actually helps."
...
Here is the part the [Retrieval](retrieval.md) lesson deferred to you, and it's the whole engine of why reranking is both *better* at judging relevance and *too expensive* to run on everything. The two stages use two structurally different kinds of model.
...
- **A recall failure is a chunking, retrieval, or [hybrid-search](hybrid-search.md) problem — upstream of the reranker.** If the right chunk isn't being retrieved at any `k`, adding a reranker changes nothing; you fix it by fixing what gets *fetched*: better chunking, a bigger candidate `k`, or hybrid search to catch the exact-term matches semantic search blurred.
- **This is why the two lessons pair.** Reranking (precision) and [hybrid search](hybrid-search.md) (recall on exact terms) attack the two *different* halves of retrieval's proxy gap; a reranker with a stronger first stage feeding it a better candidate set is worth more than a reranker alone.
...
- **Reranking is a second stage that re-scores retrieval's top candidates for true relevance and reorders them** — the targeted fix for [retrieval](retrieval.md)'s *similar-but-not-relevant* failure (a chunk that's on-topic and high-similarity but doesn't answer the question).
...
- **The mechanism is bi-encoder vs cross-encoder.** Retrieval's **bi-encoder** encodes query and document *separately*, so document vectors are precomputed and search is a cheap distance lookup. A reranker's **cross-encoder** feeds query and candidate through *together*, so [attention](../foundations/attention.md) ranges across both — far more accurate about relevance, but it can't precompute and must run once per candidate at query time.
...
- **The hard ceiling: a reranker can only reorder what retrieval already surfaced — it can never recover a chunk that wasn't retrieved.** So it fixes precision, never recall; a recall/chunking failure is an upstream problem ([chunking](chunking.md), a bigger `k`, [hybrid search](hybrid-search.md)), and no reranker touches it.
- **Rerankers come as hosted rerank APIs or open cross-encoder models**, and slot in as a stage *between* retrieval and the prompt — no change to your [vector store](vector-databases.md) or embeddings.
