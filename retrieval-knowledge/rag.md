...
A language model knows an enormous amount, but everything it knows was frozen the day its [pretraining](../foundations/pretraining-vs-posttraining.md) data was collected, and none of it is *yours*. Ask it about last week's earnings, your company's refund policy, the customer's current subscription tier, or the contents of a document you uploaded thirty seconds ago, and it has three options — all bad: say "I don't know" (useless), stay silent (also useless), or do the thing it does most naturally and invent a fluent, confident, plausible answer that happens to be wrong. That last failure is [hallucination](../reasoning-generation/hallucination.md), and it's not a bug you can prompt your way out of, because the model genuinely does not have the fact — it's answering from a closed book.

And you can't fix this by [fine-tuning](../model-behavior-training/fine-tuning.md) the facts in. That lesson's core lesson was the wall you hit here: **fine-tuning reshapes behavior, it does not reliably install knowledge** — train a model on your docs and it learns to *sound* like them while still making up the specifics. Worse, your facts change: prices update, policies get revised, a ticket's status flips from open to resolved. You are not going to retrain a model every time a database row changes.

**RAG (Retrieval-Augmented Generation) is the answer to that whole class of problem.** Instead of hoping the fact is baked into the weights, you *fetch* the relevant information at the moment of the question — from your documents, your database, a search index, the live web — and paste it into the [context window](../foundations/context-windows.md) alongside the user's question, so the model answers from an *open book* it's reading right now rather than from frozen memory. The model's job shifts from "recall the fact" to "read these passages and answer using them," which is a task it's dramatically better and safer at. This is the opening lesson of category 04, and every lesson that follows it — chunking, vector databases, retrieval, reranking, hybrid search, grounding — is a component *inside* the RAG pipeline this lesson frames.

> [!NOTE]
> **Context window** is the only prerequisite that carries weight here: it's the fixed-size working space — prompt plus retrieved text plus the model's reply — that the model can "see" at once, measured in [tokens](../foundations/tokenization.md). RAG works by loading fetched facts *into* that window. If it's fuzzy, the one-line version lives in the [ML vocabulary primer](../primers/ml-vocabulary.md); the full treatment is [Context windows](../foundations/context-windows.md). Everything else in this lesson is introduced as it appears.

## A brilliant consultant who's never seen your files

Picture hiring a consultant with an encyclopedic memory — they've read a huge slice of the world's books — but they've been locked in a room since last year and have never laid eyes on a single one of *your* company's documents. A customer asks, "What's the refund window on the annual plan?" Ask the consultant to answer from memory and you get one of three bad outcomes: a shrug, silence, or — the dangerous one — a fluent, confident "30 days" that they simply invented, because your policy was never in anything they read.

So you stop asking them to answer from memory. The instant the question comes in, an assistant darts to the filing cabinet, pulls the *Refunds* folder, and lays it open on the desk. Now the consultant doesn't recall — they *read*: "annual plans, 14-day refund window," and answer from the page. Same consultant, same intelligence; the only thing that changed is that the relevant document was on the desk when they answered.

That swap is the whole of RAG. The consultant is the language model — vast general knowledge, but frozen at training time and blind to your specifics. The assistant running to the cabinet is the *retrieval* step; the cabinet is your knowledge base; the folder laid open on the desk is the retrieved passage dropped into the context window; the consultant reading and answering is *generation*. And the two ways it goes wrong are already visible in the scene: if the assistant grabs the wrong folder, even a genius answers wrong (bad retrieval caps quality); and if you tell the consultant "answer only from what's on the desk," they stop making things up (grounding cuts hallucination).

One line to carry out of here: **RAG is open-book instead of closed-book — fetch the right pages and set them on the desk before the model answers, so it reads the facts instead of trying to recall them.** Where the picture leaks: a human consultant notices when the fetched folder is irrelevant and says "this isn't what I need"; a model usually won't — it'll gamely answer from whatever it was handed, right or wrong. The system only catches a bad fetch if you build it to.
...
- **It's expensive and slow.** You pay per [token](../foundations/tokenization.md) on every call, and latency scales with how much the model has to read. Sending 200,000 tokens of context to answer a question that needed 500 is a massive, recurring waste — see [Cost and unit economics](../production-ops/cost-unit-economics.md). Retrieval sends the model the 500 that matter.
...
This is the crossover the [fine-tuning lesson](../model-behavior-training/fine-tuning.md) pointed here to resolve, and it's the single most common RAG interview question, so carry the clean version. They are not competitors; they fix *different gaps*.
...
- **[Chunking](chunking.md)** — how you split documents into retrievable pieces. Chunk badly and the right answer gets sliced in half or buried in noise before retrieval ever runs.
- **[Vector databases](vector-databases.md)** — the store that makes "find text by *meaning*" fast at scale, using [embeddings](../foundations/embeddings.md).
- **[Retrieval](retrieval.md)** — the actual search step and how relevance is scored.
- **[Reranking](reranking.md)** — a second, sharper pass that reorders the first batch of candidates so the *best* ones land at the top.
- **[Hybrid search](hybrid-search.md)** — combining meaning-based (semantic) and keyword search, because each catches what the other misses.
- **[Grounding and citations](grounding-citations.md)** — making the model actually *use* the retrieved text and point to its source, so answers are checkable and hallucination drops.
...
- **The iron law: answer quality is capped by retrieval quality, and most RAG failures are retrieval failures, not model failures.** The whole rest of category 04 — [chunking](chunking.md), [vector DBs](vector-databases.md), [retrieval](retrieval.md), [reranking](reranking.md), [hybrid search](hybrid-search.md), [grounding](grounding-citations.md) — is a pipeline for making retrieval good.
