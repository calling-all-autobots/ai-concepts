...
Every lesson before this one handed you a lever. A bigger or [reasoning model](../reasoning-generation/reasoning-models.md) for harder problems. [Quantization](../model-behavior-training/quantization.md) and [distillation](../model-behavior-training/distillation.md) to shrink a model down. [Retrieval-Augmented Generation (RAG)](../retrieval-knowledge/rag.md) to feed in facts. [Agentic loops](../agents/agentic-systems.md) to tackle open-ended work. [Prompt caching](../prompting/prompt-caching.md), [batching](../production-ops/inference-optimization.md), [routing](../production-ops/production-architecture.md) to run it all cheaper. You now know *what each lever does*. The question this capstone answers is the one an interviewer actually asks a product manager (PM): **given all these levers, how do you decide which to pull for a specific feature?**
...
*Where it breaks:* the board makes the power budget look *fixed* — as if the only move is trading headroom between faders. That's the analogy's most dangerous seam, because it isn't strictly true. A genuinely better technique (a smarter small model, [caching](../prompting/prompt-caching.md), a tighter prompt) is like fitting a bigger amp: it raises the whole board's headroom so two faders climb at once, no trade required. The fixed-budget board is the right *default* mental model; the bigger-amp escape hatch (covered below) is what separates a good answer from a great one.

## The triangle itself

Three vertices. Define each one crisply, because interviews punish fuzziness here:

- **Quality** — is the output correct, complete, well-formatted, and safe enough for this job? What "quality" *means* is itself use-case specific and is a whole discipline on its own ([evaluation methods](../evaluation/evaluation-methods.md)); the triangle just treats it as the vertex you're often tempted to maximize.
...
- **Cost** — the money each request burns: compute time, tokens in and out, serving infrastructure. At low volume it's noise; at high volume it's the whole business ([cost and unit economics](../production-ops/cost-unit-economics.md)).
...
|---|---|---|
| Bigger vs. smaller **model choice** | [production architecture](../production-ops/production-architecture.md) | bigger → higher quality, higher cost + latency; smaller → the reverse |
| **Reasoning vs. non-reasoning** models | [reasoning models](../reasoning-generation/reasoning-models.md) | "think longer" spends latency + cost (more tokens) to buy quality on hard problems |
| **Quantization** | [quantization](../model-behavior-training/quantization.md) | shrink the weights → cheaper + faster, at some quality risk |
| **Distillation** | [distillation](../model-behavior-training/distillation.md) | train a small model to mimic a big one → cheaper + faster, aiming to keep most quality |
| **Mixture-of-Experts (MoE)** | [mixture-of-experts](../model-behavior-training/mixture-of-experts.md) | activate only part of a large model per token → big-model quality at lower compute cost |
| **Prompt caching / KV (key-value) caching** | [prompt caching](../prompting/prompt-caching.md), [KV caching](../prompting/kv-caching.md) | reuse computation on repeated context → cheaper + faster at *no* quality cost |
| **RAG** (retrieval depth) | [RAG](../retrieval-knowledge/rag.md) | fetching more/better context raises quality but adds tokens (cost) and a retrieval step (latency) |
| **Agentic depth** | [agentic systems](../agents/agentic-systems.md) | more loop iterations / self-checking → higher quality on open-ended tasks, multiplied cost + latency |
| **Self-consistency** (sample many, vote) | [sampling & decoding](../reasoning-generation/sampling-decoding.md) | run the prompt several times and take the majority → higher quality, several times the cost + latency |
| **Batching** | [inference optimization](../production-ops/inference-optimization.md) | group requests to use the hardware efficiently → cheaper, sometimes at the price of per-request latency |
| **Routing** (easy→small, hard→big) | [production architecture](../production-ops/production-architecture.md) | send each request to the cheapest model that can handle it → protects cost without capping quality |
...
- [Caching](../prompting/prompt-caching.md) makes responses both cheaper and faster with **no** quality loss — a pure outward move, not a trade.
- A better *small* model (via [distillation](../model-behavior-training/distillation.md) or a new architecture) can match last year's big model at a fraction of the cost and latency — the whole triangle relocates.
- A tighter [prompt](../prompting/prompt-caching.md) or better [retrieval](../retrieval-knowledge/rag.md) can raise quality *and* cut cost by sending fewer, more relevant tokens.
...
4. **Define and measure the vertices.** "Quality" needs an [evaluation](../evaluation/evaluation-methods.md); "cost" needs [unit economics](../production-ops/cost-unit-economics.md); latency needs a target. A tradeoff you can't measure is a tradeoff you can't defend.
