...
*Where it breaks:* a real intern learns across the day, genuinely understands the goal, and has the common sense to notice when they're off track. An LLM agent has no memory unless you [build it in](35-agent-memory.md), can't reliably verify its own work, and will compound a wrong turn without noticing — so it's a more literal, more brittle "intern" than the picture flatters it to be.
...
1. **Goal in.** The user's objective enters the context, along with the available [tools](32-tool-calling.md).
2. **Reason.** The model thinks about the current state — what's been done, what's left (this is where [chain-of-thought](../03-reasoning-generation/17-chain-of-thought.md) reasoning earns its keep, deciding the next move).
...
- **Cost and latency multiply.** Every loop iteration is a model call, and the context grows with each appended result — so a ten-step agent is many model calls over an ever-longer prompt. (This is exactly why [prompt caching](../05-prompting/31-prompt-caching.md) and lean tool results matter for agents.)
- **Runaway behavior.** Agents can loop, get stuck re-trying, or pursue a wrong subgoal enthusiastically. You need hard guardrails: step/iteration caps, spending budgets, and — for any irreversible action — human approval, since the [security seam from tool calling](32-tool-calling.md) is now wider (more calls, chosen autonomously, some steerable by [prompt injection](../08-safety-trust/42-ai-security.md)).
...
- The parts around the loop — [memory](35-agent-memory.md), [planning/orchestration](36-planning-orchestration.md), [multi-agent](37-multi-agent.md) — have their own lessons; the loop is the core.
