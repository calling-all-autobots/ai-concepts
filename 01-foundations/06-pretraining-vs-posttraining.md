...
**The picture:** A brilliant graduate who has read the entire library — they know a staggering amount, but they blurt facts, ignore the question you actually asked, and have no manners. Then they take a short customer-service course that teaches them to listen, answer what was asked, and be polite. They learn no new facts in that course — only how to behave.

**The mapping:** reading the whole library = pretraining (where knowledge is acquired); the raw graduate = the base model; the customer-service course = post-training (where behavior is shaped); the polished employee = the instruct/chat model you actually talk to.

**Why it holds:** the core insight is that knowledge and behavior are installed in two separate stages, and the second changes conduct, not facts — and "education, then a manners course that adds no new knowledge" is the everyday sequence that separates acquiring knowledge from learning how to use it. That's exactly the pretraining/post-training split.

**Say it like this:** "Pretraining is reading the whole library; post-training is the manners course on top — knowledge from the first, behavior from the second, and you can't fix missing knowledge with more manners."

*Where it breaks:* a real person keeps learning facts on the job; the model's knowledge is essentially frozen at pretraining — post-training barely adds any.
...
- **[Instruction tuning](../02-model-behavior-training/10-instruction-tuning.md)** — teaching the model to follow instructions, typically the first post-training step.
- **[Fine-tuning](../02-model-behavior-training/07-fine-tuning.md)** — the general practice of further training a model on a targeted dataset to specialize its behavior.
- **[RLHF (Reinforcement Learning from Human Feedback)](../02-model-behavior-training/11-rlhf.md)** — shaping the model using human preferences about which of its answers are better.
- **[DPO (Direct Preference Optimization)](../02-model-behavior-training/12-dpo.md)** — a newer, simpler way to achieve a similar preference-shaping result.
...
- **[Fine-tuning](../02-model-behavior-training/07-fine-tuning.md), [RLHF](../02-model-behavior-training/11-rlhf.md), [DPO](../02-model-behavior-training/12-dpo.md), and [instruction tuning](../02-model-behavior-training/10-instruction-tuning.md) are all post-training techniques** — they all sit under this frame.
