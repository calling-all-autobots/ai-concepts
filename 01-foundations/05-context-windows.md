...
> [!NOTE]
> **Token** — models don't read characters or whole words; they read *tokens*, which are word-fragments (roughly ¾ of a word each in English). The context window is measured in tokens, not words or characters, which is why its size is always quoted as a token count. Tokens have their own lesson — see [Tokenization](01-tokenization.md); here, just know the window is counted in them.

## The one analogy to remember

**The picture:** The model works entirely off a single fixed-size whiteboard. Everything it needs — the standing instructions, the conversation so far, your question, and room to write its answer — has to fit on that one board, and the board is wiped clean after every question.

**The mapping:** the whiteboard's size = the context window (in tokens); what's written on it = the system prompt + conversation history + your input + reserved output space; wiping it after each question = the model being stateless; re-copying the old notes back on each time = the app re-sending the whole transcript.

**Why it holds:** the two things people get wrong are that the window is a *fixed, shared* budget and that the model *keeps nothing* between calls — and a whiteboard is the everyday object that is both at once: fixed-size (everything competes for the space) and wiped clean (nothing carries over). It nails both load-bearing truths in one image.

**Say it like this:** "The model thinks on a fixed-size whiteboard that's wiped after every question — if something isn't written on the board right now, the model has no idea about it."

*Where it breaks:* a whiteboard is read equally everywhere; a model reads the middle of a very full board *less* reliably — the "lost in the middle" effect.
...
The sharp framing: the context window is the model's **field of view**. It can reason brilliantly about anything inside the frame and is completely blind to anything outside it. Nothing you told it yesterday, nothing from three calls ago, nothing in your database is "known" to the model unless it is physically present in the window *for this call*.
...
- **"Just tell it to remember X for later."** It can't. There is no "later" that carries state. Anything that must persist across calls has to be written down somewhere by your system and re-injected into the window when it's next needed — which is exactly the job of [Agent memory](../06-agents/35-agent-memory.md). Persistence across calls is an application feature, not a model feature; the model itself is stateless.
...
> [!NOTE]
> **Attention** — the mechanism a transformer uses to let each token weigh how much every other token matters to it. It's what lets the model use context at all, and it's also the source of the cost curve below. It has its own lesson — see [Attention](03-attention.md); here we only borrow two of its results.
...
One real mitigation for the *latency* half of this problem is **KV (key/value) caching**: the model can cache the internal work it already did for the unchanged front of the prompt (the system prompt, the earlier turns) so it doesn't recompute it every call. It softens the "re-read from the top" penalty but doesn't remove the underlying cost of a large window, and it has its own lesson — [KV caching](../05-prompting/30-kv-caching.md).
...
The alternative is **retrieval**: instead of sending everything, fetch only the few passages relevant to *this* question and put just those in the window. That's the job of [RAG (Retrieval-Augmented Generation)](../04-retrieval-knowledge/21-rag.md), and it's linked here because the *how* belongs to that lesson — what matters for context windows is knowing *when* to reach for it.
...
- The context window is the model's **field of view for one call** — the max tokens (measured in [tokens](01-tokenization.md), not words) it can consider at once. It's blind to everything outside the frame.
- It's **working memory, not long-term memory.** The model is stateless between calls; the *feeling* of a continuous chat is the application re-sending the whole transcript every turn, at full cost. Persistence across calls is an app feature — see [Agent memory](../06-agents/35-agent-memory.md).
...
- **A bigger window isn't free.** Cost and latency rise with the *square* of the length (the quadratic attention cost — see [Attention](03-attention.md)), and you pay it per call on every token, relevant or not.
- **Window vs. retrieval** is decided by volatility and slice-size: static and needed-whole → window; large, changing, or query-a-slice → [retrieval/RAG](../04-retrieval-knowledge/21-rag.md). A bigger window raises the ceiling; it doesn't make context free.
